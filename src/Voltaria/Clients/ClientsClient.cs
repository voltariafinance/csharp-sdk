...
public partial class ClientsClient : IClientsClient
{
    ...
    private async Task<
        WithRawResponse<PaginatedResponseClientUserResponse>
    > ListClientPortalUsersAsyncCore(
    ...
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                var responseData = JsonUtils.Deserialize<PaginatedResponseClientUserResponse>(
                    responseBody
                )!;
                return new WithRawResponse<PaginatedResponseClientUserResponse>()
                {
                    Data = responseData,
                    RawResponse = new RawResponse()
                    {
    ...
    /// );
    /// </code></example>
    public WithRawResponseTask<PaginatedResponseClientUserResponse> ListClientPortalUsersAsync(
        ListClientPortalUsersRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<PaginatedResponseClientUserResponse>(
            ListClientPortalUsersAsyncCore(request, options, cancellationToken)
        );
    }
